...
2. Implementar controller com endpoints:
   ...
   export class TgflocController {
     ...
     async findByProduct(@Param('codprod') codprod: number): Promise<LocalEstoque[]> {
...
1. Criar novo arquivo `locations-api.ts`:
   ...
   export const locationsApi = {
     ...
     async getByProduct(codprod: number): Promise<LocalEstoque[]> {
       const response = await axios.get(`${API_BASE_URL}/tgfloc/with-stock/${codprod}`);
       return response.data;
     },
   };
   ```
...
1. Adicionar novos métodos ao `productService`:
   ...
   export const productService = {
      ...
      */
     async getProductsWithLocations(params: ProductSearchParams & { codlocal?: number }) {

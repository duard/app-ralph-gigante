  ...
  persist(
    (set, get) => ({
      ...
      updateProduct: (id, updates) =>
        set((state) => ({
          products: state.products.map((p) => (p.codprod === id ? { ...p, ...updates } : p)),
          selectedProduct:
      ...
      removeProduct: (id) =>
        set((state) => ({
          ...
          selectedProduct: state.selectedProduct?.codprod === id ? null : state.selectedProduct,
        })),
      ...
      getFilteredProducts: () => {
        ...
        // Filter by category
        if (filters.category) {
          filtered = filtered.filter((p) => p.descrgrupoprod === filters.category);
        }

        // Filter by unit
        if (filters.unit) {
          filtered = filtered.filter((p) => p.codvol === filters.unit);
        }

        // Filter by price range
        if (filters.priceMin !== undefined) {
          filtered = filtered.filter((p) => (p.vlrvenda ?? 0) >= (filters.priceMin ?? 0));
        }
        if (filters.priceMax !== undefined) {
          filtered = filtered.filter((p) => (p.vlrvenda ?? 0) <= (filters.priceMax ?? Infinity));
        }
      ...
      getActiveProductsCount: () => get().products.filter((p) => p.ativo === 'S').length,

      getInactiveProductsCount: () => get().products.filter((p) => p.ativo === 'N').length,

      getTotalStockValue: () =>
        get().products.reduce((total, p) => total + (p.estoque ?? 0) * (p.vlrvenda ?? 0), 0),
    }),
    {

...
export function CommandSearch({ open, onOpenChange }: CommandSearchProps) {
  ...
  const performProductSearch = React.useCallback(
    async (query?: string) => {
      if (!query || !query.trim()) {
        setProductResults([]);
        setIsLoading(false);
  ...
  const { debounced: debouncedProductSearch, cancel } = useDebounce(performProductSearch, 300, {
    leading: false,
    trailing: true,
  ...
  const groupedItems = allSearchItems.reduce(
    (acc, item) => {
      if (!acc[item.group]) {
        acc[item.group] = [];
      }
          ...
          <CommandList>
            {isLoading && (
              <div className="flex items-center justify-center h-12">
                <LoadingState type="spinner" size="sm" message="Buscando produtos..." />
              </div>
            )}

            {!isLoading && searchQuery.trim() && productResults.length === 0 && (
              <CommandEmpty>Nenhum produto encontrado para "{searchQuery}"</CommandEmpty>
            )}
